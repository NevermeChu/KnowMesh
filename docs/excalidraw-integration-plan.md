...
- 创建对话框增加文档类型选择，根节点和子节点共用同一流程。
- 所有导航 DTO、路径、面包屑、收藏、最近访问和搜索结果携带 `kind`。
- 侧边栏使用不同图标，混合类型树仍支持懒加载、移动和删除。
- `Document` 改为判别联合，读取路径按授权与类型加载载荷。
- 新增只读白板组件，验证深链打开和无权限状态不泄露 scene。

验收：富文本下可建白板、白板下可建富文本；跨项目移动整棵混合子树后类型与载荷不变；viewer 与非项目成员符合现有权限边界。
