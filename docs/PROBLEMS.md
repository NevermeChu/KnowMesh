...
## 66. CI 注入环境变量后协作服务仍因缺少 `.env` 无法启动

### 问题

GitHub Actions 的真实 PostgreSQL E2E 已通过容器环境注入数据库、协作端口和认证变量，但 Hocuspocus 在测试开始前以 `.env: not found` 退出，Playwright 只能报告 WebServer 启动失败。

### 根因

本地运行器无条件向 `tsx` 传入 `--env-file=.env`。该参数要求文件实际存在，忽略了 CI 和生产可以直接通过进程环境提供完整配置，且仓库按安全边界不会提交 `.env`。
...
自动部署依赖已经存在的 `current` 回滚目标、`knowmesh.service`、完整 Nginx 站点、数据库、证书、部署用户和 sudoers。现场审计已经取得当前配置，但仓库仍只版本化协作 systemd unit 与 Nginx 片段；空服务器无法仅凭仓库建立与当前生产一致的运行环境，服务器迁移和灾难恢复依赖现场导出。
...
## 69. 生产 PostgreSQL 没有可验证的备份与恢复链路

### 问题

生产 PostgreSQL 16 当前在服务器本机运行，但现场未发现已启用的 `pg_dump` 或 `pg_basebackup` timer、相关 cron、专用备份目录、异地副本、保留策略、RPO/RTO 或恢复演练证据。VM 或磁盘故障可能同时丢失运行数据库和所有本机状态。

### 根因

现有 CI/CD 只负责迁移和应用 release 回滚，数据库持久性被当作服务器外部前置条件，没有建立独立的备份目标、调度、失败告警与恢复验证。

### 解决方法

- 在实际开放前确定异地备份目标、加密方式、保留周期、RPO 和 RTO，使用受控凭据自动生成并复制备份。
- 对备份任务、备份新鲜度、容量和失败建立告警；本机副本只作为临时恢复点，不能作为唯一备份。
- 在隔离环境定期执行恢复演练，记录用时和校验结果；只有真实恢复通过后才能声称生产数据可恢复。

## 70. 生产部署账户与网络边界没有落实最小权限

### 问题

现场 `thisme` 拥有 `NOPASSWD: ALL`，可获得任意 root shell；SSH 仍允许 root 公钥登录。UFW 未启用且 iptables INPUT 默认允许，Azure NSG 的实际规则未确认；Next.js unit 也缺少协作 unit 已使用的基础 systemd 沙箱。

### 根因

服务器沿用 cloud-init 的管理员默认权限，CI deploy 通过远程执行内联 root shell 完成 release 解压、迁移、软链接切换和服务控制，因此尚未把部署动作收敛为可最小授权的稳定入口。主机与云侧防火墙、SSH 和进程沙箱也没有形成统一的版本化基线。
