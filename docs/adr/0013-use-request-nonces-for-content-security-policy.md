...
- 根布局从内部请求头读取 nonce，并把它附加到主题初始化脚本；Next.js 从 CSP 自动为框架脚本应用同一 nonce。
- 现有 React `style` 属性仍需要 `style-src 'unsafe-inline'`；脚本策略不因此放宽。
...
- 新增外部脚本、连接目标、字体或图片来源时必须显式审查并更新 CSP，不能临时加入宽泛脚本来源。
