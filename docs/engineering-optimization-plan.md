...
**C3. 成员、邀请与转让失效面：已核对并保留。** Workspace 邀请接受、Project 邀请接受、成员移除及所有权转让可改变当前用户的 Workspace/Project 集合、角色或导航能力；其余审批和角色动作复用同一成员事务边界。第一轮不拆分这些失效调用，避免把目标用户可见集合与当前操作者局部 UI 错当成同一刷新语义。

批次 C 本地验证：lint、`check:types`、`build:next` 通过；文档写入与导航测试 26/26、相关 Chromium 回归 4/4 通过。项目、Workspace 及成员治理动作的 layout 失效保持不变。
