...
通知属于 Clerk 用户，而不属于当前选中的 Workspace。用户切换 Workspace 后仍能查看自己的全部通知；服务端读取和已读写入始终使用当前 Clerk `userId` 限制收件人，客户端不能指定收件人身份。
...
- `recipient_user_id` 是收件人的 Clerk 用户 ID，也是所有读取和已读写入的隔离条件。
- `actor_user_id` 是可选的触发者 Clerk 用户 ID；触发者删除账户后会被置空。
- `type` 表示稳定的业务事件类型。
...
索引分别支持按收件人与创建时间读取列表，以及按收件人统计未读通知。KnowMesh 当前没有本地用户表，因此收件人和触发者都不使用数据库用户外键。

Clerk `user.deleted` 清理会删除该用户收到的全部通知，并把其他用户通知中的该用户触发者引用置空。
