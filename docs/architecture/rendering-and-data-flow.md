...
- 当前存在独立 Hocuspocus 双向 WebSocket 服务与客户端 Provider；本地普通启动已按功能开关完成编排，CI 已在真实 PostgreSQL 与协作服务下通过，Nginx 和 systemd 尚未接入，生产功能开关必须保持关闭。
