...
const state = vi.hoisted(() => {
  ...
  return { document, getProjectAccess, insert, protect, returning, values };
});
...
describe('document creation action', () => {
  ...
  it('creates document in editable project', async () => {
    await expect(createDocument({ projectId: state.document.projectId })).resolves.toStrictEqual(
      state.document,
    );
  ...
  it('rejects viewer creation', async () => {
    ...
    await expect(createDocument({ projectId: state.document.projectId })).rejects.toThrow(
      '没有权限在该项目中创建文档',
    );
